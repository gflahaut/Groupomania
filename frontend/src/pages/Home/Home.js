...
function Home() {
  ...
  React.useEffect(() => {
    ...
    Axios.post(`http://localhost:5000/auth/role`, { userid: userid})
      .then((response) => {
  ...
  React.useEffect(() => {
    ...
    return(
      <div>
        <div className="hfilters">
          <label htmlFor="hfilter" className="hfilterLabel">Rechercher les posts d'un utilisateur</label>
          <input id="hfilter" type="text" placeholder="Ex: Jacques Lachery ou jlachery" onChange={getUsers}/>
          <span className="clearFilters" onClick={clearFilters}><FaMinusCircle/> Clear filter</span>
        </div>
        <div className="userHolder">
          {usersList.map((val, index) => {
            return( 
              <div key={index} data-user={val.userId} onClick={filterPosts}>{val.username+" - "+ val.firstname+" "+val.name}</div>
            )
          })}
        </div>
        <div>
                ...
                return(
                  <li className="h" key={index}>
                    <div className="hPosts" id="marker1-xs">
                      <input type="hidden" className="hidPost" placeholder={val.idposts}/>
                      <input type="hidden" className="hidAuthor" placeholder={val.postuserid}/>
                      <div className="Image">
                        {val.imageurl}
                        <img className="himg" alt="post" src={Avatar}/>
                      </div>
                      <div className="hTitle visible-xs-block" id="marker-xs">{val.title}</div>
                      ...
                      <div className="hLikes">{val.likes.split(",").length}<FaThumbsUp data-post={ val.idposts } data-likes={val.likes} className="thumbsUp" onClick={ postLiked }/></div> 
                    </div>
                  </li>
                );
              })};
                ...
                return(
                  <li className='h' key={index}>
                    <div className="hPosts" id="marker1-md">
                      <input  id="home" type= "hidden" className="hidPost" placeholder={val.idposts}/>
                      <input  id="home" type= "hidden" className="hidAuthor" placeholder={val.postuserid}/>
                      <div className="Image">
                        {val.imageurl}
                        <img className="himg"alt="post" src={Avatar}/>
                      </div>
                      ...
                      <div className="hDescription">{val.description}</div>
                      <div className="hDate">created in {val.date}</div>
                      <div className="hLikes" data-post={ val.idposts } data-likes={val.likes} onClick={ postLiked }>{val.likes.split(",").length}
                      ...
                      {userRole == "ADMIN" ? (
                        <>
                        <button className="Buttons btn btn-danger delete col-md-8 offset-md-2 mt-4 visible-xs-block" data-post={ val.idposts } onClick={ confirmDelete }>Delete</button>
                        </>
                      ) : ("")}
                      </div>
                  </li>
    ...
    function getUsers(){
      ...
      if(search.length > 2){
        Axios.post(`http://localhost:5000/auth/filter`, {search: search})
          .then((response) => {
    ...
    function filterPosts(e){
      ...
      Axios.post(`http://localhost:5000/posts/fromUser`, {userid: userid})
        .then((response) => {
          setPostsList(response.data);
        });
    }
    ...
    function postLiked(e){
      ...
      console.log(postId);
      console.log(likes);
      Axios.post('http://localhost:5000/posts/like',{postId: postId, userid: userId, likes: likes})
      .then((response)=> {
        console.log("??")
          setRefresh(!refresh);
      })
